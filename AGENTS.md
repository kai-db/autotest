...
5. **修复走 agent-dev-loop** — 分析问题/修复代码**不直接改**，必须走 `agent-dev-loop` skill（**启动方 agent 为 driver** 负责计划/实现/修复/记录，**另一方 agent 为 reviewer** 只读独立 review：从 Codex 打开则 Codex driver + Claude reviewer，从 Claude Code 打开则反之；task 内不换角）：建 `docs/implementation/YYYY-MM-DD-动词-对象/` 任务目录 → plan → reviewer plan review → 实现 → reviewer 实现 review → 回写 results.md。修复原则见 `TEST_GUIDE.md` 第三节，闭环步骤见第六节
