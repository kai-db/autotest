...
**测/修严格分离**——测试阶段只测不修，跑完全量才统一修，这是防卡死的核心。

```
Phase 1: 确认环境（设备在线 → App 可启动）
Phase 2: 全量测试（**只测不修**，按 P0→P1→P2 遍历用例，每条重置+截图验证）
  └── 遇 FAIL：agent-dev-loop「分析记录模式」登记证据（≤10min，不改代码）→ 立刻下一条
Phase 3: 更新文档（结果写入 results.md，先记录再修复）
Phase 4: 全部修复（**走 agent-dev-loop**：续跑任务目录 → plan → reviewer review → 实现 → reviewer review）
Phase 5: 回归测试（全量重跑，不只跑 FAIL 的；规则同 Phase 2）
  └── 仍有 FAIL → 回到 Phase 3
