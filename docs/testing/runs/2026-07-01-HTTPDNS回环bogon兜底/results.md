...
| TC-N-06 | 备用域名真异线路核对 | ❌ **FAIL(真机实锤,2026-07-01 补)** | 真机 S25 直查所在网络路由器权威 DNS(绕 fake-IP 代理):**`ws.dbxsocial.com` 是 `ws.debox.pro` 的 CNAME 别名**(同 `ws.debox.pro.eo.dnse5.com`、同 IP `43.168.24.175/20.116`)→ **非真异线路**,主被污染时备用同 CNAME 一起挂。= BUG-002 真因,需运维重配。详见 BUG-002 |
